...
#' `!is.na(rate)`): with an empty subset `den_share` reads `0` where some
#' jurisdiction reported the denominator and `NA` where none did, so neither
#' test alone catches both.
#'
